...
interface TemplatesGetRulesResponse {
    templates: string[];
}

interface TemplatesAddRulePayload {
    yaml: string
}
...
export class TemplatesController extends Controller {
    @Get("")
    public async getTemplates(@Request() request: ExRequest): Promise<TemplatesGetRulesResponse> {
    ...
    @Post("{id}")
    public async addTemplate(@Request() request: ExRequest, @Route("id") id: string, @Body() body: TemplatesAddRulePayload): Promise<TempatesAddResponse> {
        let server: ElastalertServer = request.app.get('server');
        let templatesService = server.templatesService;
